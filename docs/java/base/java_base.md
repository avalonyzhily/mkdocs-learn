...
### Disruptor 消费者响应策略
BlockingWaitStrategy:默认策略，利用锁和条件进行数据监控和线程唤醒,节省CPU,但是会涉及到线程的频繁切换,性能比较差。

SleepingWaitStrategy:优先自选等待,不成功再使用Thread.yield()让出CPU进行线程休眠,会有较高的平均延时。

YieldingWaitStrategy:循环监控缓冲区数据变化,使用Thread.yield()让出CPU,延时低,但是对CPU的占用相当高(内部执行Thread.yield()死循环),要求逻辑CPU数量(X核Y线程中的Y)要多于消费者线程数

BusySpinWaitStrategy:死循环监控缓冲区数据变化,一直占用CPU,0延时才考虑使用的策略,而且要求物理CPU个数要大于消费者线程数(X核Y线程中的X)

### 伪共享问题
CPU的高速缓存中读写数据最小单位是缓存行,因此一个以上的变量存在一个缓存行时,多线程访问会产生影响性能,cpu1上的线程修改其中一个变量会导致该缓存行对其他cpu失效,无法命中缓存,然后其他cpu上的线程修改了某个变量,又会导致cpu1的缓存失效,这样就会互相影响。

### Disruptor的优化
在变量前后填充一定的位置,使得该变量独占一行,避免为共享。

## 但是要注意JDK的实现不同,这个方案不一定好用！！！比如jdk8对不适用的变量会有优化,导致填充方案失效。


###  Future模式
Future 是一个契约，可以异步调用获取结果,用于拿到结果

Callable 用于产生结果

FutureTask 集成了两者,开启新的线程去执行Callable产生结果,并立即返回future对象给当前线程作为契约,让当前线程在需要的时候去获取由Callable生成的结果

### 并行流水线
并行流水线是处理有数据依赖关系的多个步骤的并行设计,可以有效的提高性能,前提是多次任务的情况,因为单次任务又有前后依赖,只能串行一次;
而并行流水线的意义在于,将步骤拆分后,连续的串行任务可以让各个步骤可以不停的接受和处理任务。

(这里可以借用BlockingQueue,没有任务是就等待任务下达,不会一直占用cpu的阻塞,而且本身支持多线程并发的线程安全)

### 是否使用多线程
任务的阻塞系数，阻塞系数高于50%的属于io行任务，线程的数量可以大于核心数，小于50的属于计算密集型，最好线程数控制在核心线程数。
