...
- 各种引擎对锁的支持不一样
    ...
    - 间隙锁的理解,对于条件范围内不存在的记录也会加锁,因此叫间隙锁,这是为了满足mysql基于sql的binlog日志在恢复和复制时保证数据正确的需要,同时也可以有效的防止幻读
        - 重点!!!5.6版本以上(目前大佬测试,低于5.6未确认),间隙锁只有在Serializable隔离级别才生效,而不是因为恢复和复制的需要而在RC和RR2个隔离级别也生效
        - 重点的重点!!!前面说的是隔离级别的实现,当使用LOCK IN SHARE MODE 或 FOR UPDATE这种主动加锁的指令后,就已经无视当前隔离级别了,因此间隙锁也会生效
